...
class RednaoPDFGenerator
{
    ...
    public function __construct($options,$useTestData=false,$order)
    {
    ...
    {
        global $wpdb;
        $invoiceDataList = $wpdb->get_results("SELECT invoice_id,attach_to,options,html,extensions,conditions,pages FROM ".RednaoWooCommercePDFInvoice::$INVOICE_TABLE." WHERE attach_to LIKE '%" . $wpdb->esc_like($emailId) . "%'");
    ...
    public function GenerateAttachment($path,&$attachments,$index)
    {
    ...
    public function Generate($getFromDatabase=false,$skipDomPDFRenderer=false,$readOnly=false)
    {
        ini_set('display_errors', 0);
        $this->dompdf = new Dompdf();
        $this->dompdf->set_option('enable_remote', TRUE);
        $this->dompdf->getOptions()->setTempDir(RednaoWooCommercePDFInvoice::$DIR.'vendor/dompdf/dompdf/temp')->setIsFontSubsettingEnabled(true);
       ...
       /* echo $html;
        ...
        if($getFromDatabase)
        {
            ...
            if($result!=false)
            {
                ...
                $this->formattedInvoiceNumber=$result->formatted_invoice_number;
        ...
        if(!$alreadyRecorded)
        {
            ...
            $this->date=$data->Date;
            $this->formattedInvoiceNumber=$data->FormattedInvoiceNumber;
        }
...
/*
    ...
    public function render(){
        ...
        $size=$this->pageType;
        if($size=='custom')
            $size=array(0,0,$this->pageWidth,$this->pageHeight);
        $this->dompdf->setPaper($size, $this->orientation);
        $this->dompdf->render();
    }
     ...
     */
    private function MaybeFilterOrder($order)
    {
