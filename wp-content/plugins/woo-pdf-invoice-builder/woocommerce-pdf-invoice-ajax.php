...
final class RednaoWooCommercePDFInvoiceAjax{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_rednao_wcpdfinv_save_custom_field',array($this,'SaveCustomField'));
    ...
    public function  SaveNextNumber(){
        ...
        update_option($invoiceId. '_rednao_pdf_invoice_number',$nextNumber);
    ...
    public function Download(){
        ...
        if(count($invoiceList)==1)
        {
            ...
            foreach($invoiceList as $invoice)
            {
                ...
                require_once 'PDFGenerator.php';
                $generator=new RednaoPDFGenerator(RednaoPDFGenerator::GetPageOptionsById($invoice->InvoiceId),false,$order);
                $generator->Generate(true,true);
    ...
    public function Search(){
        ...
        if($customerName!='')
        {
            $where.=' and user.display_name like \'%'.$wpdb->esc_like($customerName).'%\'';
        }

        if($invoiceNumber!='')
        {
            $where.=' and formatted_invoice_number like \'%'.$wpdb->esc_like($invoiceNumber).'%\'';
        }
    ...
    public function EmailPDF(){
      ...
      //  RednaoWooCommercePDFInvoice::CheckIfPDFAdmin();
        ...
        if($order==false)
        {
            ...
            $generator=new RednaoPDFGenerator($options,false,$order);
    ...
    public function SearchInvoice(){
        ...
        foreach($results as &$result)
        {
            ...
            $result['ViewUrl']=wp_specialchars_decode(wp_nonce_url( admin_url( "admin-ajax.php?action=rednao_wcpdfinv_generate_pdf&orderid=" . $result['OrderNumber'] ), 'rednao_wcpdfinv_generate_pdf' ));
    ...
    public function CatchShutdownHandler($errorNumber, $errorStr,$errorFile,$errorLine,$errorContext){
    ...
    public function SaveCustomField(){
        $id=$this->GetStringValue('id',true);
        $name=$this->GetStringValue('name',true);
        $code=$this->GetStringValue('code',true);
        $type=$this->GetStringValue('type',true);

        if(!is_super_admin())
        {
            $this->SendErrorMessage('Only admins can edit custom fields');
        }
        ...
        if($id==0||$id==null)
        {
            ...
            $result=$wpdb->insert(RednaoWooCommercePDFInvoice::$CUSTOM_FIELDS_TABLE,array(
                'custom_field_name'=>$name,
                'custom_field_text'=>$code,
                'custom_field_type'=>$type
            ));
            $rowId=$wpdb->insert_id;
        }else{
            $result=$wpdb->update(RednaoWooCommercePDFInvoice::$CUSTOM_FIELDS_TABLE,array(
                'custom_field_name'=>$name,
                'custom_field_text'=>$code,
                'custom_field_type'=>$type
        ...
        if($result===false)
        {
            $this->SendErrorMessage('Data could not be inserted');
        }
        else
        {
    ...
    public function Export(){
        ...
        $invoiceData=$wpdb->get_results("select extensions,conditions,attach_to,invoice_id,name,options,type,html,pages from ".RednaoWooCommercePDFInvoice::$INVOICE_TABLE." where invoice_id=".$_POST['pageId'],"ARRAY_A");
        if(count($invoiceData)<=0){
            return;
        }
        $options=json_decode($invoiceData[0]['options'],true);
        foreach($options['fieldOptions'] as &$option)
        {
        ...
        echo json_encode(array(
            'html'=>$invoiceData[0]['html'],
            'name'=>$invoiceData[0]['name'],
            'pageType'=>$invoiceData[0]['type'],
            'attach_to'=>$invoiceData[0]['attach_to'],
            'extensions'=>$invoiceData[0]['extensions'],
            'conditions'=>$invoiceData[0]['conditions'],
            'pages'=>$invoiceData[0]['pages'],
            'pageId'=>0,
    ...
    public function CheckIfOrderIsValid()
    {
        ...
        if($post==null||$post->post_type!='shop_order')
    ...
    public function CreatePDF(){
        ...
        if(wp_verify_nonce($_GET['_wpnonce'], 'rednao_wcpdfinv_generate_pdf')==false){
            die('Forbidden');
        }
