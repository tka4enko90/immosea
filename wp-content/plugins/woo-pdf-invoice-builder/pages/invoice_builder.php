...
if(isset($_GET['id']))
{
    $invoiceData = $wpdb->get_results($wpdb->prepare("SELECT invoice_id,attach_to,name,options,type,html,extensions,conditions,pages,my_account_download,create_when FROM ".RednaoWooCommercePDFInvoice::$INVOICE_TABLE." WHERE invoice_id=%s",$_GET['id']));
}else{
    $invoiceData=array();
}
...
wp_localize_script('wcrbc-pdfbuilder-react','rednaoPDFInvoiceParams',array(
    ...
    'Statuses'=>$statusList,
    'Emails'=>$emailList,
    'LatestNumber'=>get_option($id. '_rednao_pdf_invoice_number',0),
