...
class WC_VAT_Validator {
			...
			if ( ( $this->input != NULL ) && ( ! empty( $this->input[ 0 ] ) ) && ( ! empty( $this->input[ 1 ] ) ) ) {
				...
				if ( $this->hasClient ) {
					...
					if ( ( $today_api_calls < 4 ) && ( $today_total_api_calls < 25000 ) ) {
						...
						$error_log = apply_filters( 'german_market_vat_id_verify_error_log', false );
						
						if ( $error_log ) {
							
							if ( $today_api_calls >= 4) {
								error_log( 'GERMAN MARKET VAT VALIDATOR ERROR: "A Vat ID has already been validated 4 times today as invalid" CHECK REGEX INSTEAD' );
							}

							if ( $today_api_calls >= 25000) {
								error_log( 'GERMAN MARKET VAT VALIDATOR ERROR: "More than 25.000 API Calls today" CHECK REGEX INSTEAD' );
							}
	...
	private function check_API() {
		...
		try {
			...
			$error_log = apply_filters( 'german_market_vat_id_verify_error_log', false );
			if ( $error_log ) {
				error_log( 'GERMAN MARKET VAT VALIDATOR ERROR: "' . $e->getMessage() . '" CHECK REGEX INSTEAD' );
			}
